...
import {createMessage, useAppDispatch, useAppSelector} from "../store";
...
export const MessageInput: FC<Props> = ({recipientId}) => {
	...
	return (
			<form className="w-full flex items-center justify-between p-2 shadow-2xl fixed bottom-0 bg-white">
				{isPlaceholderNeeded && (
				...
				<div className="m-2 absolute right-2">
					{text ?
							...
							<button onClick={() => createMessage(text, recipientId)}>
								<svg className="text-inherit fill-current h-6" viewBox="0 0 24 24">
									<path
